...
def inSafetyZone(position):
    
    # Check if the robot is inside a safety zone defined by a circle with pre-defined radius 
    radius = 10
    dist = math.sqrt(position[0]*position[0] + position[1]*position[1] + position[2]*position[2])
    if dist > radius:
        stdscr.addstr(25, 25, 'THE ROBOT IS OUTSIDE THE SAFETY ZONE!!!')
    else:
        stdscr.addstr(25, 25, 'OK. Robot is inside the safety zone')
